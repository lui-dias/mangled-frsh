...
function createMangleClassNames() {
	...
	return {
		name: 'mangle-class-names',
		setup(build) {
			build.onLoad({
				filter: /.+\.tsx/,
			}, (args) => {
				let text = Deno.readTextFileSync(args.path)

				Deno.writeTextFileSync(`${n}-old.tsx`, text)

				for (const [k, v] of classesMap.entries()) {
					text = text.replaceAll(k, v)
				}

				Deno.writeTextFileSync(`${n++}-new.tsx`, text)
