...
import postcssRaname from 'npm:postcss-rename'

const DEFAULT_OPTIONS = {
	content: ['./**/*.tsx'],
	theme: {},
} as tailwindcss.Config
...
export const bundle = async (
	{ to, from, map }: { to: string; from: string; map: Map<string, string> },
) => {
	...
	const processor = postcss([
		...
		{
			postcssPlugin: 'class-minify',
			prepare: () => {
				return {
					Rule(r) {
						const className = r.selector.replaceAll('\\', '').slice(1)
						if (map.has(className)) {
							console.log(r.selector, '.' + map.get(className)!)
							r.selector = '.' + map.get(className)!
						}
					},
				}
			},
	...
	console.info(
		` 🎨 Tailwind css ready in ${cyan(`${((performance.now() - start) / 1e3).toFixed(1)}s`)}`,
	)
}
